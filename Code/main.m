...
%         Elayer(i).T=TRUST(alpha0,beta0,Elayer(i).Nsa,Elayer(i).Na,Elayer(i).Nsc,Thn,(sum(Elayer(i).T(1,1:N1))-Elayer(i).T(i))/(N1-1),Elayer(i).id);
        Elayer(i).T=TRUST(alpha0,beta0,Elayer(i).Nsa,Elayer(i).Na,Elayer(i).Nsc,Thn,Elayer(i).trust,Elayer(i).id);
        % 平均求出综合信任值
        Elayer(i).trust=(sum(Elayer(i).T)-Elayer(i).T(i))/(Elayer(i).NerNode-1);

        if Elayer(i).EHistory~=0
            Elayer(i).trust=CHECK(Elayer(i).trust, Elayer(i).trustB,Eta, Elayer(i).EHistory);
        end
        if Elayer(i).Err~=0
            Elayer(i).trust=min(Elayer(i).trust, trust_Thn(Elayer(i).Err));
            Elayer(i).EHistory=Elayer(i).Err;
        end

        Elayer(i).trustB=Elayer(i).trust;
    end
...
%         Mlayer(i).Req=ones(1,Mlayer(i).NerNode);
        ...
        if Mlayer(i).EHistory~=0
            Mlayer(i).trust=CHECK(Mlayer(i).trust, Mlayer(i).trustB,Eta, Mlayer(i).EHistory);
        end
        if Mlayer(i).Err~=0
            Mlayer(i).trust=min(Mlayer(i).trust, trust_Thn(Mlayer(i).Err));
            Mlayer(i).EHistory=Mlayer(i).Err;
        end

        Mlayer(i).trustB=Mlayer(i).trust;
